...
{% hint style="info" %}
1.idip回调命令字：需要游戏侧研发提供，一般是增加一个for问卷回调的命令字，如果命令字未登记过，需要找到--AMS腾讯游戏营销平台助手小M报备

例如：

申请应用： IEG-AMS-11836\
使用场景：问卷数据回调\
申请业务：代号xx（在ams中登记的名称）\
修改类命令：详细说明命令字（例如10xxx5）

2.idip接口key固定无需变更

3.ams平台业务缩写：填写游戏对应登记的名称

4.接口自定义参数：查看下方详细说明填入

5.idip回调环境：与当前问卷投放游戏环境一致
