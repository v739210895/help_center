...
问卷完成编辑及设置后，有问卷审核权限的用户可通过“发布”功能使投放链接生效，进行问卷投放；若用户无问卷审核权限，需要提交审核到指定审核人进行问卷审核，审核通过后投放链接自动生效。样本收集完成后，用户可以通过“暂停回收”功能关闭投放链接，以确保问卷信息安全。

## 发布

问卷完成编辑及设置后，建议使用预览链接发给组内同事及所在项目组成员协助检查，确保无误后再开始回收。在问卷编辑页内点击右上角”发布“或通过”我的问卷—操作—发布”点击确定即可。
...
**预发布：**&#x9002;用于希望快速验证问卷效果并减少审核等待的场景。可跳过审核环节， 进行小范围测试（例如验证逻辑设定、登录验证、数据回收情况），测试完成后可暂停预发布状态进入后续的提审/正式投放环节。

**正式发布：**&#x9002;用于问卷测试完成并准备正式收集数据的场景。正式发布后，投放链接可正常回收答题数据。
{% endhint %}
